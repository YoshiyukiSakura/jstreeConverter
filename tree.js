...
(function (factory) {
	 ...
	 */
	$.jstree.convert = function (json, option){
		this.opts = $.extend({text:"name", id:"id", children:false, selected:1}, option);
		var tree = [];//初始化tree
		var key,subkey,grandson;

		for ( key in json ) {//一次遍历取出目录
			tree[key] = {"text":json[key][this.opts.text]+"&nbsp;&nbsp;&nbsp;&nbsp;"
						,"id":json[key][this.opts.id]
						,"state" : { "opened":true,"disabled" : true }
						,"children":[]};
			if (this.opts.children)
				{
					for ( subkey in json[key][this.opts.children]) {//二次遍历取出节点
						    grandson = {//每个节点的对象
							 "text":json[key][this.opts.children][subkey][this.opts.text]
							,"id":json[key][this.opts.children][subkey][this.opts.id]
							,"state":{"selected":(json[key][this.opts.children][subkey][this.opts.id]==this.opts.selected)}//默认选中
						};
						tree[key].children.push(grandson);
					};
				};
		};
		return tree;
